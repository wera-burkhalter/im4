...
// Event-Daten laden
$stmt = $pdo->prepare("SELECT e.title, e.description, e.image, e.date, e.time, e.place, e.deadline, b.firstName AS organizerFirstName, b.surname AS organizerSurname FROM events e JOIN benutzer b ON e.benutzer_id = b.id WHERE e.id = ?");
...
// Eingeladene Gäste
$stmt = $pdo->prepare("SELECT b.firstName, b.surname, b.profilbild FROM event_has_benutzer ehb JOIN benutzer b ON ehb.benutzer_id = b.id WHERE ehb.event_id = ?");
$stmt->execute([$eventId]);
